...
class AskModelChat(APIView):
    def post(self,request,*args,**kwargs):
        ...
        messages = MessageModel.objects.filter(user=user)
        message_serializer = MessageModelSerializer(messages, many=True)
        all_messages = message_serializer.data
        user_messages = all_messages[-4:]
        ...
        elif text_string.startswith("Laghub:"):
            response = text_string[len("Laghub:"):].lstrip()
        elif text_string.startswith("System:"):
            response = text_string[len("System:"):].lstrip()
